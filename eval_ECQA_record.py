...
class PintoDataset(Dataset):
    ...
    def __init__(
        self, problems, explanations, tokenizer, source_len, target_len, args, diff_Pattern = False, OnlyAnswer = False
    ):
        """
   ...
   args, model_params, output_dir="./outputs/"
    ...
    def run(run_set,run_problems,prefix):
        ...
        for _, pair in tqdm(enumerate(run_set)):
            ...
            gt_sol = explanations[problem['id']]['explanation']
            if args.dataset == "strategyqa":
                question = problem['context']
            else:   
                question = problem['question']
    ...
    dir = args.answer_extractor_path.replace('pytorch_model.bin',f'cases_{solution_giver_part}_{args.use_pipeline}')
    if not os.path.exists(dir):
        os.makedirs(dir)
    
    if args.do_eval:
        prefix = args.answer_extractor_path.replace('pytorch_model.bin',f'cases_{solution_giver_part}_{args.use_pipeline}/eval')
        print(prefix)
        correct_log = open(log_path,'a')
    ...
    if args.do_train:
        prefix = args.answer_extractor_path.replace('pytorch_model.bin',f'cases_{solution_giver_part}_{args.use_pipeline}/train')
        print(prefix)
        correct_log = open(log_path,'a')
    ...
    if args.do_test:
        prefix = args.answer_extractor_path.replace('pytorch_model.bin',f'cases_{solution_giver_part}_{args.use_pipeline}/test')
        print(prefix)
        correct_log = open(log_path,'a')
