...
if __name__ == '__main__': 
    ...
    model_dir = f"./Models/ScienceQA/verifier/{args.model}_{train_strategy}_{timestamp}_{args.lr}"
    logging_dir = f"./Models/ScienceQA/verifier/{args.model}_{train_strategy}_{timestamp}_{args.lr}/logs"
...
class ScienceQADataset(Dataset):
    ...
    def __init__(
        self, problems, qids, tokenizer, source_len, target_len, split, args, diff_Pattern = False, OnlyAnswer = False
    ):
        """
        ...
        self.label = []
        self.source_text = []
        if args.train_strategy=='generated':
            wrong_cases_ = set()
            wrong_cases = json.load(open(os.path.join(args.generated_data_dir, f'{split[:-4]}_wrong_cases.json')))

            for case in wrong_cases:
                qid = qids[case['num']]
                wrong_cases_.add(qid)
            ...
            right_cases = json.load(open(os.path.join(args.generated_data_dir, f'{split[:-4]}_right_cases.json')))
            counter = 0
            for case in right_cases:
                qid = qids[case['num']]
                if qid in wrong_cases_:
                    continue
                counter += 1
                question, context, choice, answer, lecture, solution = build_data(problems, qid, args)
                _solution = case["prediction solution"].replace('Solution: ','')
                #_solution = case["ground truth solution"].replace('Solution: ','')
                _prompt = f"Question: {question}\nContext: {context}\nOptions: {choice}\nBECAUSE: {_solution}\n"
                self.label.append(1)
                self.source_text.append(_prompt)
            correct_log = open(log_path,'a')
            correct_log.write('negative:'+str(len(wrong_cases)))
            correct_log.write('positive:'+str(counter))
            correct_log.close()
            print('negative:',len(wrong_cases))
        ...
        for qid in self.data:
            ...
            if args.train_strategy=='random_sample':
                while True:
                    sid = random.choice(qids)
                    if sid != qid: 
                        break
                _problem = problems[sid]
                _solution = _problem['solution'].replace("\n", "\\n")
                _prompt = f"Question: {question}\nContext: {context}\nOptions: {choice}\nBECAUSE: {_solution}\n"
                self.label.append(0)
                self.source_text.append(_prompt)
            cnt+=1
...
def T5Trainer(
    ...
    def model_init():
        if args.use_bert:
            verifier = AutoModelForSequenceClassification.from_pretrained("bert-large-cased", num_labels=2)
        else:    
