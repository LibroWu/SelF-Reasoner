...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='csqa', choices=['csqa'])
...
class PintoDataset(Dataset):
    ...
    def __init__(
        self, problems, explanations, tokenizer, source_len, target_len, args, OnlyAnswer = False
    ):
        """
...
def compute_metrics(eval_preds):
    ...
    #print(logits.shape)
    #print(labels[0])
    #preds = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True) for g in logits[:5]]
    #target = [tokenizer.decode(t, skip_special_tokens=True, clean_up_tokenization_spaces=True)for t in labels[:5]]
    #print("preds",preds)
    #print("target",target)
    #print(compute(predictions=labels, references=labels))
...
if __name__ == '__main__':
    ...
    model_dir = f"./Models/ECQA/{args.dataset}/{model_name}_{prompt_format}_{timestamp}"
